...
public class ControladorCoche {
	...
	public Coche findSiguiente(int idActual) {
		...
		Query q = em.createNativeQuery("select * from tutorialjavacoches.coche where id > ? order by id limit 1",
				Coche.class);
	...
	public Coche findAnterior(int idActual) {
		...
		Query q = em.createNativeQuery("select * from tutorialjavacoches.coche where id < ? order by id desc limit 1",
				Coche.class);
		q.setParameter(1, idActual);
		c = (Coche) q.getSingleResult();
		em.close();
